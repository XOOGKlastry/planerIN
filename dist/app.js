...
let state=defaultState(),view='week',selectedDay=0,busy=false,map=null,miniMap=null,toastTimer,search='',filter='all',selectedVisit=null,geoCandidates=[],geoTarget=null,importPreview=null,waitingWorker=null,deferredInstall=null,cancelGeo=false,saveError=null,addSectionExpanded=false,pane='list',openStop=null,markers=new Map(),showDone=false,showLater=false,myPosition=null,meLayer=null;
...
function nextStopCandidates(day){
  ...
  const pool=currentVisits().filter(v=>!placed.has(v.id)&&validCoords(v.location)&&!v.blocked&&!v.done&&!v.later);
...
const finishedAt=v=>Math.max(0,...v.jobs.map(j=>Date.parse(j.finishedAt)||0));
...
function render(){
  ...
  const jobs=selectedJobs(),vs=currentVisits(),unready=vs.filter(v=>!v.done&&!v.later&&(!validCoords(v.location)||v.blocked)),p=currentPlan();
...
function dayTile(day){
  const r=dayRoute(day),s=dayDriving(day),overnight=!!currentPlan()?.overnight?.[day],doneCount=r.filter(v=>v.done).length,left=r.length-doneCount,allDone=r.length>0&&left===0;
  const total=s.hasMissing?null:s.driving+(overnight?0:s.back);
  const meta=!r.length?'':allDone?`${icon('checkCircle')}<span>gotowe</span>`:doneCount?`${icon('checkCircle')}<span>${doneCount}/${r.length}</span>`:`${icon('clock')}<span>${total===null?'—':shortDuration(total)}</span>`;
  const label=`${DAY_NAMES[day]}, ${formatDate(addDays(state.settings.week,day))}: ${allDone?'wszystko zrobione':`${left} ${plural(left,'przystanek','przystanki','przystanków')} do zrobienia`}${doneCount&&!allDone?`, ${doneCount} zrobione`:''}${overnight?', nocleg':''}`;
  return `<button class="day-tab ${day===selectedDay?'active':''} ${allDone?'all-done':''}" style="--day-color:${DAY_COLORS[day]}" data-act="day" data-day="${day}" aria-pressed="${day===selectedDay}" aria-label="${esc(label)}"><span class="day-tab-head"><b>${DAY_SHORT[day]}</b><span>${Number(addDays(state.settings.week,day).slice(8))}</span>${overnight?`<i class="day-tab-flag">${icon('moon')}</i>`:''}</span><span class="day-tab-count">${allDone?icon('check'):left||'–'}</span><span class="day-tab-meta">${meta}</span></button>`;
}
function renderWeek(vs,p,unready){
  ...
  const missing=vs.filter(v=>!v.done&&!v.later&&!validCoords(v.location)).length,blocked=vs.filter(v=>!v.done&&!v.later&&v.blocked).length;
...
function renderDayBar(day,route,summary,distance,overnight){
  const hasCost=!!dayCostContext(day).cost,done=route.filter(v=>v.done).length,left=route.length-done;
  ...
  return `<div class="daybar" style="--day-color:${DAY_COLORS[day]}"><div class="daybar-title"><span class="day-dot" aria-hidden="true"></span><h2>${DAY_NAMES[day]}<small>${formatDate(addDays(state.settings.week,day))}</small></h2></div><div class="daybar-stats">${stat('pin',left,'Do zrobienia')}${stat('clock',total===null?'—':shortDuration(total),'Łącznie w drodze')}${distance?stat('road',`${Math.round(distance/1000)} km`,'Dystans'):''}${overnight?stat('moon','nocleg','Koniec dnia'):stat('home',back===null?'—':shortDuration(back),'Powrót do bazy',back>=60?'warn':'')}${route.length?stat('checkCircle',`${done}/${route.length}`,'Zrobione',!left?'ok':done?'ok-part':''):''}</div><div class="daybar-tools">${distances}${routeUrl?`<a class="tool" href="${esc(routeUrl)}" target="_blank" rel="noopener noreferrer" title="Prowadź całą trasę w Google Maps" aria-label="Prowadź całą trasę w Google Maps">${icon('route')}</a>`:''}${route.length?`<button class="tool" data-act="reset-day" title="Wyczyść dzień" aria-label="Wyczyść dzień">${icon('trash')}</button>`:''}</div><button class="primary optimize" data-act="optimize-day" ${busy||left<2?'disabled':''} title="Ułóż pozostałe przystanki w najkrótszej kolejności">${icon('wand')}<span>Optymalizuj</span></button><div class="pane-switch" role="group" aria-label="Widok dnia">${[['list','list','Lista'],['map','map','Mapa']].map(([id,sym,label])=>`<button class="${pane===id?'active':''}" data-act="pane" data-pane="${id}" aria-pressed="${pane===id}">${icon(sym)}<span>${label}</span></button>`).join('')}</div></div>`;
}
...
function renderRoute(day,route,summary,distance,overnight){
  ...
  const routeIds=new Set(route.map(v=>v.id)),stops=route.map((v,i)=>v.done?'':leg(summary.entries.find(e=>e.visit?.id===v.id)?.travel)+renderStop(v,i,routeIds)).join('');
  const end=route.length?`${overnight?'':leg(summary.back)}${renderRouteEnd(day,overnight,route,distance)}`:'';
  return `<div class="timeline">${startRow}${renderDoneGroup(route)}${stops}${renderAddSection(day)}${end}</div>`;
}
// Finished stops leave the active route: they fold into one green "Zrobione" line at the top (the part of the
// day already behind the crew) and open into a compact, struck-through list with an undo for each stop.
function renderDoneGroup(route){
  const done=route.map((v,i)=>({v,i})).filter(({v})=>v.done);
  if(!done.length)return '';
  const all=done.length===route.length;
  const when=v=>{const t=finishedAt(v);return t?` · ${new Date(t).toLocaleString('pl-PL',{weekday:'short',hour:'2-digit',minute:'2-digit'})}`:'';};
  const rows=showDone?done.map(({v,i})=>`<div class="tl-row done-row"><div class="tl-gutter"><span class="tl-node ok" title="Przystanek ${i+1} — zrobiony">${icon('check')}</span></div><div class="tl-main"><strong>${esc(locationTitle(v.location))}</strong><small>${esc(v.location.label)}${when(v)}</small></div><div class="tl-side"><button class="chip-action undo" data-act="toggle-done" data-id="${esc(v.id)}" title="Cofnij — przystanek wraca do zrobienia">${icon('undo')}<span>Cofnij</span></button></div></div>`).join(''):'';
  return `<div class="tl-row tl-done-head ${all?'all':''}"><div class="tl-gutter"><span class="tl-node ok">${icon('check')}</span></div><div class="tl-main"><button class="done-summary" data-act="toggle-done-list" aria-expanded="${showDone}"><strong>${all?'Wszystko zrobione':'Zrobione'}<span class="count ok">${done.length}/${route.length}</span>${icon(showDone?'up':'down')}</strong><small>${done.map(({v})=>esc(v.location.code||locationTitle(v.location))).join(' · ')}</small></button></div></div>${rows}`;
}
...
function renderMapToolbar(route){
  const placed=placedVisitIds(),parked=currentVisits().some(v=>v.later&&!placed.has(v.id));
  return `<div class="map-toolbar"><div class="map-legend"><span><i class="lg-pin route">1</i>Ten dzień</span><span><i class="lg-pin free">+</i>Wolne</span><span><i class="lg-pin other"></i>Inny dzień</span>${route.some(v=>v.done)?'<span><i class="lg-pin done">✓</i>Zrobione</span>':''}${parked?'<span><i class="lg-pin later">↺</i>Na później</span>':''}</div><div class="map-tools"><button class="tool quiet" data-act="locate-me" title="Gdzie jestem" aria-label="Pokaż moją lokalizację na mapie">${icon('target')}</button><button class="tool quiet" data-act="fit-map" title="Pokaż całość" aria-label="Pokaż całość">${icon('expand')}</button>${route.length?`<button class="tool quiet" data-act="load-route" title="Odśwież przebieg trasy" aria-label="Odśwież przebieg trasy">${icon('refresh')}</button>`:''}<button class="tool quiet" data-act="materials-route" title="Składy kruszywa po drodze" aria-label="Składy kruszywa po drodze">${icon('mountain')}</button></div></div>`;
}
...
function renderStop(v,index,routeIds){
  const status=v.done?'done':v.jobs.some(j=>j.status==='doing')?'doing':'todo',near=proximityWarning(v.id,other=>!routeIds.has(other.id)&&!other.done&&!other.later),open=openStop===v.id,id=esc(v.id),title=esc(locationTitle(v.location));
  const flags=[
  ...
  // Green / amber / red, side by side and spelled out: done, put aside for later, delete the order for good.
  const stateAct=(a,cls,sym,label,hint)=>`<button class="state ${cls}" data-act="${a}" data-id="${id}" title="${hint}">${icon(sym)}<span>${label}</span></button>`;
  const statusRow=`<div class="status-row">${stateAct('toggle-done','ok','check',v.done?'Cofnij':'Zrobione','Zrobione — przystanek wypada z trasy')}${stateAct('mark-later','later','hourglass','Na później','Odłóż na później — wypada z trasy, ale zostaje na liście')}${stateAct('delete-visit','danger','trash','Usuń zlecenie','Kasuje zlecenie z telefonu — bez powrotu')}</div>`;
...
function renderCandidate(v,rank,best,routeIds,lastId){
  const located=validCoords(v.location),near=routeIds.size?proximityWarning(v.id,other=>routeIds.has(other.id)&&!other.done&&other.id!==lastId):null,id=esc(v.id),title=esc(locationTitle(v.location));
  const flags=[
...
function renderAddSection(day){
  const placed=placedVisitIds(),all=currentVisits();
  const pool=all.filter(v=>!placed.has(v.id)&&!v.done&&!v.later),laterPool=all.filter(v=>!placed.has(v.id)&&v.later);
  const route=dayRoute(day),routeIds=new Set(route.map(v=>v.id)),lastId=route.at(-1)?.id,hasStops=routeIds.size>0,later=renderLaterGroup(laterPool);
  if(!pool.length)return (hasStops||laterPool.length?'':`<div class="tl-row tl-next-head"><div class="tl-gutter"><span class="tl-node ok">${icon('checkCircle')}</span></div><div class="tl-main"><strong>Wszystko zaplanowane</strong><small>Każde zlecenie tego tygodnia ma już swój dzień.</small></div></div>`)+later;
...
// Parked work: it left the day, not the phone. Folded to one amber line; a tap plans it back onto the open day.
function renderLaterGroup(pool){
  if(!pool.length)return '';
  const rows=showLater?pool.map(v=>{const id=esc(v.id),title=esc(locationTitle(v.location));
    return `<div class="tl-row later-row"><div class="tl-gutter"><button class="cand-add later" data-act="plan-later" data-id="${id}" ${v.blocked||!validCoords(v.location)?'disabled':''} title="Wróć do planu — dodaj do tego dnia" aria-label="Zaplanuj ${title} w tym dniu">${icon('plus')}</button></div><div class="tl-main"><button class="stop-head" data-act="visit" data-id="${id}"><strong>${title}</strong><span>${esc(v.location.label)}</span></button></div><div class="tl-side"><button class="tool danger-tool" data-act="delete-visit" data-id="${id}" title="Usuń zlecenie z telefonu" aria-label="Usuń zlecenie ${title}">${icon('trash')}</button></div></div>`;}).join(''):'';
  return `<div class="tl-later"><div class="tl-row tl-later-head"><div class="tl-gutter"><span class="tl-node later">${icon('hourglass')}</span></div><div class="tl-main"><button class="later-summary" data-act="toggle-later-list" aria-expanded="${showLater}"><strong>Na później<span class="count later">${pool.length}</span>${icon(showLater?'up':'down')}</strong><small>${pool.map(v=>esc(v.location.code||locationTitle(v.location))).join(' · ')}</small></button></div></div>${rows}</div>`;
}
...
// Done sinks to the bottom, parked work sits just above it, so the top of the list is what is still on.
const jobRank=j=>j.status==='done'?2:j.status==='later'?1:0;
function renderJobs(jobs){const rows=jobs.filter(j=>{const l=state.locations[j.locationId],matches=norm(`${l.code} ${l.label} ${j.type} ${j.details}`).includes(norm(search));return matches&&(filter==='all'||filter==='missing'&&!validCoords(l)||filter==='confirm'&&j.requiresConfirmation&&!j.confirmed||filter==='todo'&&j.status!=='done'&&j.status!=='later'||filter==='done'&&j.status==='done'||filter==='later'&&j.status==='later');}).sort((a,b)=>jobRank(a)-jobRank(b));return `${weekSelector()}<div class="toolbar"><input id="search-jobs" type="search" placeholder="Kod, adres lub opis pracy" value="${esc(search)}" aria-label="Szukaj zleceń"><select id="job-filter" aria-label="Filtr zleceń">${[['all','Wszystkie'],['todo','Do wykonania'],['later','Na później'],['done','Wykonane'],['missing','Brak lokalizacji'],['confirm','Do potwierdzenia']].map(([id,label])=>`<option value="${id}" ${id===filter?'selected':''}>${label}</option>`).join('')}</select><span class="filter-count">${rows.length} pozycji</span></div><section class="panel table-wrap"><table class="data-table"><thead><tr><th>Paczkomat / adres</th><th>Praca</th><th class="hide-mobile">Status</th><th></th></tr></thead><tbody>${rows.map(j=>{const l=state.locations[j.locationId],v=currentVisits().find(v=>v.jobs.some(a=>a.id===j.id));return `<tr class="${j.status==='done'?'row-done':j.status==='later'?'row-later':''}"><td><strong>${j.status==='done'?`<span class="done-mark" title="Wykonano">${icon('checkCircle')}</span>`:''}${esc(locationTitle(l))}</strong><small>${esc(l.label)}</small></td><td>${esc(j.type)}${j.requiresConfirmation&&!j.confirmed?'<br><span class="tag amber">Potwierdź wyjazd</span>':''}${!validCoords(l)?'<br><span class="tag amber">Brak lokalizacji</span>':''}</td><td class="hide-mobile">${j.status==='done'?'<span class="tag green">Wykonano</span>':j.status==='later'?'<span class="tag amber">Na później</span>':j.status==='doing'?'<span class="tag blue">W trakcie</span>':'Do wykonania'}</td><td><div class="row-actions"><button class="small" data-act="visit" data-id="${esc(v.id)}">Otwórz</button><button class="small icon danger" data-act="delete-visit" data-id="${esc(v.id)}" title="Usuń zlecenie" aria-label="Usuń zlecenie ${esc(locationTitle(l))}">${icon('trash')}</button></div></td></tr>`;}).join('')||'<tr><td colspan="4" style="padding:40px;text-align:center">Brak zleceń w tym widoku.</td></tr>'}</tbody></table></section>`;}
...
function drawMap(){
  ...
  for(const v of currentVisits()){
    ...
    if(v.done&&!inRoute)continue;
    const p=[v.location.lat,v.location.lng];points.push(p);
    const otherDay=inRoute?undefined:dayOfVisit.get(v.id),index=inRoute?route.findIndex(r=>r.id===v.id)+1:null,parked=!inRoute&&v.later;
    const dayClass=inRoute?`day${selectedDay}`:parked?'later':otherDay!==undefined?`day${otherDay}`:'candidate';
    const label=inRoute?(v.done?'✓':index):parked?'↺':otherDay!==undefined?DAY_SHORT[otherDay]:'+';
    const marker=L.marker(p,{icon:L.divIcon({className:'',html:`<div class="map-pin ${dayClass} ${v.done?'done':''} ${v.blocked&&!v.done?'pending':''} ${v.id===selectedVisit?'active':''}">${label}</div>`,iconSize:[32,32],iconAnchor:[16,16]})}).addTo(map);
    markers.set(v.id,marker);
    const moveNote=parked?`Odłożone na później. Kliknij, żeby wrócić z tym do ${DAY_NAMES[selectedDay]}.<br>`:otherDay!==undefined?`Zaplanowane na ${DAY_NAMES[otherDay]}. Kliknij, żeby przenieść na ${DAY_NAMES[selectedDay]}.<br>`:'Kliknij pinezkę, żeby dodać jako kolejny przystanek.<br>';
    marker.bindPopup(`<strong>${esc(locationTitle(v.location))}</strong><br>${esc(v.location.label)}<br>${inRoute?(v.done?'Zrobione.<br>':''):moveNote}<a href="${esc(googleMapsUrl(v.location))}" target="_blank" rel="noopener noreferrer">Google Maps</a>`);
    marker.on('click',async()=>{
      if(inRoute){if(!v.done&&openStop!==v.id)setOpenStop(v.id);return;}
      if(parked){await planLater(v.id);return;}
  ...
  if(points.length)map.fitBounds(points,{padding:[35,35],maxZoom:15,animate:false});
  meLayer=null;drawMyPosition(map,false);
...
// Only the stops still to do are reordered, starting from where the crew is now — the most recently finished
// stop (or the day's start while nothing is done). Finished stops stay at the front, in the order they were done.
async function optimizeDayOrder(day){
  ...
  const route=dayRoute(day),done=route.filter(v=>v.done).sort((a,b)=>finishedAt(a)-finishedAt(b)),open=route.filter(v=>!v.done);
  if(open.length<2)return toast('Za mało przystanków do zrobienia, żeby coś optymalizować.');
  const indexOf=v=>vs.findIndex(a=>a.id===v.id)+1,indices=open.map(indexOf),from=done.length?indexOf(done.at(-1)):anchor;
  if(indices.some(i=>i<=0)||from<0)return toast('Brakuje danych o dojeździe do części przystanków. Policz odległości ponownie.');
  const optimized=exactRoute(indices,cost,from);
  const p=currentPlan();if(!p)return;
  p.routes[day]=[...done.map(v=>v.id),...optimized.map(i=>vs[i-1].id)];
  state.plans[planKey()]=p;await save();loadDayRoute();
  toast(done.length?'Pozostałe przystanki ułożone od ostatniego zrobionego.':'Kolejność zoptymalizowana.');
}
...
function removeFromRoutes(id){let removed=false;for(const plan of Object.values(state.plans))for(const ids of Object.values(plan.routes||{})){const i=ids?.indexOf(id)??-1;if(i>=0){ids.splice(i,1);removed=true;}}return removed;}
async function removeStop(id){removeFromRoutes(id);await save();}
...
// Up/down swaps with the nearest stop still to do — done ones sit folded away, so swapping with one would look like nothing happened.
async function moveBy(id,direction){const route=currentPlan()?.routes?.[selectedDay];if(!route)return;const done=new Set(dayRoute(selectedDay).filter(v=>v.done).map(v=>v.id)),i=route.indexOf(id);let j=i+direction;while(j>=0&&j<route.length&&done.has(route[j]))j+=direction;if(i<0||j<0||j>=route.length)return;[route[i],route[j]]=[route[j],route[i]];await save();loadDayRoute();}
// One tap marks every job at the stop done (or back to do); the stop then leaves the active route list.
async function toggleDone(id){
  const v=currentVisits().find(v=>v.id===id);if(!v)return;
  const makeDone=!v.done,now=new Date().toISOString();
  for(const job of v.jobs){if(makeDone){job.status='done';job.startedAt??=now;job.finishedAt=now;}else{job.status='todo';job.startedAt=null;job.finishedAt=null;}}
  if(openStop===id)openStop=null;
  await save();
  toast(makeDone?`Zrobione: ${locationTitle(v.location)}.`:`${locationTitle(v.location)} wraca do zrobienia.`);
}
// Amber: the crew is not doing this now. It drops out of the day and out of the suggestions, keeps its
// description and history, and waits in the folded "Na później" group until someone plans it again.
async function markLater(id){
  const v=currentVisits().find(v=>v.id===id);if(!v)return;
  const park=!v.later;
  for(const job of v.jobs){if(job.status==='done')continue;job.status=park?'later':'todo';job.startedAt=null;job.finishedAt=null;}
  if(park)removeFromRoutes(id);
  if(openStop===id)openStop=null;
  await save();
  toast(park?`Na później: ${locationTitle(v.location)}. Znajdziesz je pod „Na później”.`:`${locationTitle(v.location)} wraca do planowania.`);
}
async function planLater(id){
  const v=currentVisits().find(v=>v.id===id);if(!v)return;
  for(const job of v.jobs)if(job.status==='later')job.status='todo';
  if(await addStop(selectedDay,id)){toast(`${locationTitle(v.location)} wraca do trasy.`);loadDayRoute();}
  else await save();
}
// Red: the order is gone from this phone — jobs, location and its place in every plan. Asked for in full
// first, because no import file is kept afterwards; only a fresh Excel (or a colleague's plan) brings it back.
function confirmDeleteVisit(id){
  const v=currentVisits().find(v=>v.id===id);if(!v)return;
  const n=v.jobs.length;
  showModal('Usunąć zlecenie?',`<p>Z telefonu zniknie <strong>${n} ${plural(n,'zlecenie','zlecenia','zleceń')}</strong> w punkcie <strong>${esc(locationTitle(v.location))}</strong>:</p><ul class="delete-list">${v.jobs.map(j=>`<li>${esc(j.type)}${j.ticket?` · ${esc(j.ticket)}`:''}</li>`).join('')}</ul><p class="privacy-text">Punkt wypada z trasy i z zakładki Zlecenia. Tego nie da się cofnąć — wróci dopiero z ponownym importem Excela. Jeśli chcesz tylko odłożyć pracę, wybierz <strong>Na później</strong>.</p>`,`<button data-act="close">Anuluj</button><button class="delete-confirm" data-act="delete-visit-confirm" data-id="${esc(id)}">${icon('trash')}Usuń zlecenie</button>`);
}
async function deleteVisit(id){
  const v=currentVisits().find(v=>v.id===id);if(!v)return;
  const jobIds=new Set(v.jobs.map(j=>j.id)),title=locationTitle(v.location),locationId=v.locationId;
  state.jobs=state.jobs.filter(j=>!jobIds.has(j.id));
  removeFromRoutes(id);
  if(!state.jobs.some(j=>j.locationId===locationId))delete state.locations[locationId];
  pruneMatrix(id);
  if(openStop===id)openStop=null;if(selectedVisit===id)selectedVisit=null;
  modal.close();await save();
  toast(`Zlecenie usunięte: ${title}.`);
}
// Drops the deleted point's row and column instead of throwing the whole matrix away, so one deletion
// doesn't cost the crew another round of OSRM requests.
function pruneMatrix(removedId){
  const m=currentPlan()?.matrix;if(!m)return;
  const keep=m.visitIds.map((visitId,i)=>visitId===removedId?-1:i).filter(i=>i>=0);
  if(keep.length===m.visitIds.length)return;
  const rows=[0,...keep.map(i=>i+1)],slice=grid=>Array.isArray(grid)?rows.map(r=>rows.map(c=>grid[r]?.[c])):grid;
  m.visitIds=keep.map(i=>m.visitIds[i]);m.durations=slice(m.durations);m.distances=slice(m.distances);
  m.key=matrixKey(state.settings.base,matrixVisits());
}
// "Gdzie jestem": one GPS fix, a blue dot with its accuracy ring. The fix is kept for the session so the
// dot survives every re-render of the map, and nothing is sent anywhere — Leaflet draws it locally.
function locateMe(){
  if(!navigator.geolocation)return toast('Ta przeglądarka nie udostępnia lokalizacji.');
  toast('Szukam Twojej lokalizacji…');
  navigator.geolocation.getCurrentPosition(position=>{
    myPosition={lat:position.coords.latitude,lng:position.coords.longitude,accuracy:position.coords.accuracy};
    drawMyPosition(map,true);drawMyPosition(workMap,true);
    toast(`Jesteś tutaj — dokładność około ${Math.round(myPosition.accuracy)} m.`);
  },error=>toast(error.code===1?'Brak zgody na lokalizację. Włącz ją dla tej strony w ustawieniach telefonu.':'Nie udało się odczytać lokalizacji. Sprawdź GPS i spróbuj ponownie.'),{enableHighAccuracy:true,timeout:15000,maximumAge:15000});
}
function drawMyPosition(target,center){
  if(!target||!myPosition||!globalThis.L)return;
  if(target===map&&meLayer){meLayer.remove();meLayer=null;}
  const layer=L.layerGroup([
    L.circle([myPosition.lat,myPosition.lng],{radius:Math.max(20,Math.min(myPosition.accuracy||40,600)),color:'#2f6fed',weight:1,fillColor:'#2f6fed',fillOpacity:.1}),
    L.marker([myPosition.lat,myPosition.lng],{icon:L.divIcon({className:'',html:'<div class="map-me" role="img" aria-label="Twoja lokalizacja"></div>',iconSize:[22,22],iconAnchor:[11,11]}),zIndexOffset:1000}).bindPopup('<strong>Tu jesteś</strong>')
  ]).addTo(target);
  if(target===map)meLayer=layer;
  if(center)target.setView([myPosition.lat,myPosition.lng],Math.max(target.getZoom()||0,14),{animate:false});
}
...
async function saveVisit(form){const v=currentVisits().find(v=>v.id===form.dataset.visit);if(!v)return;const f=new FormData(form),now=new Date().toISOString();for(const job of v.jobs){job.confirmed=f.has(`confirmed:${job.id}`);const status=f.get(`status:${job.id}`);if(status!==job.status){job.status=status;if(status==='doing')job.startedAt??=now;if(status==='done')job.finishedAt=now;if(status==='todo'||status==='later'){job.startedAt=null;job.finishedAt=null;}}}if(currentVisits().find(a=>a.id===v.id)?.later)removeFromRoutes(v.id);modal.close();await save();toast('Zlecenia zapisane.');}

async function exportPlan(){
  const copy=structuredClone(state);delete copy.importResult;
  const blob=new Blob([JSON.stringify(copy,null,2)],{type:'application/json'}),name=`szlaq-${state.settings.week}.json`,file=new File([blob],name,{type:'application/json'});
  if(navigator.canShare?.({files:[file]})){try{await navigator.share({files:[file],title:'Plan ekipy — szlaq'});return;}catch(e){if(e.name==='AbortError')return;}}
...
document.addEventListener('click',async event=>{
  ...
  try{switch(action){
    ...
    case 'day':selectedDay=Number(button.dataset.day);addSectionExpanded=false;openStop=null;showDone=false;showLater=false;render();loadDayRoute();break;
    ...
    case 'toggle-done':await toggleDone(id);break;
    case 'toggle-done-list':showDone=!showDone;render();break;
    case 'mark-later':await markLater(id);break;
    case 'plan-later':await planLater(id);break;
    case 'toggle-later-list':showLater=!showLater;render();break;
    case 'delete-visit':confirmDeleteVisit(id);break;
    case 'delete-visit-confirm':await deleteVisit(id);break;
