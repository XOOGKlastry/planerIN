...
function nextStopCandidates(day){
  ...
  const pool=currentVisits().filter(v=>!placed.has(v.id)&&validCoords(v.location)&&!v.blocked&&!v.done);
...
function render(){
  ...
  const jobs=selectedJobs(),vs=currentVisits(),unready=vs.filter(v=>!v.done&&(!validCoords(v.location)||v.blocked)),p=currentPlan();
...
function renderWeek(vs,p,unready){
  ...
  const missing=vs.filter(v=>!v.done&&!validCoords(v.location)).length,blocked=vs.filter(v=>!v.done&&v.blocked).length;
...
function renderStop(v,index,routeIds){
  const status=v.done?'done':v.jobs.some(j=>j.status==='doing')?'doing':'todo',near=proximityWarning(v.id,other=>!routeIds.has(other.id)&&!other.done),open=openStop===v.id,id=esc(v.id),title=esc(locationTitle(v.location));
  const flags=[
...
function renderAddSection(day){
  const placed=placedVisitIds(),pool=currentVisits().filter(v=>!placed.has(v.id)&&!v.done),route=dayRoute(day),routeIds=new Set(route.map(v=>v.id)),lastId=route.at(-1)?.id,hasStops=routeIds.size>0;
...
function renderJobs(jobs){const rows=jobs.filter(j=>{const l=state.locations[j.locationId],matches=norm(`${l.code} ${l.label} ${j.type} ${j.details}`).includes(norm(search));return matches&&(filter==='all'||filter==='missing'&&!validCoords(l)||filter==='confirm'&&j.requiresConfirmation&&!j.confirmed||filter==='todo'&&j.status!=='done'||filter==='done'&&j.status==='done');}).sort((a,b)=>(a.status==='done')-(b.status==='done'));return `${weekSelector()}<div class="toolbar"><input id="search-jobs" type="search" placeholder="Kod, adres lub opis pracy" value="${esc(search)}" aria-label="Szukaj zleceń"><select id="job-filter" aria-label="Filtr zleceń">${[['all','Wszystkie'],['todo','Do wykonania'],['done','Wykonane'],['missing','Brak lokalizacji'],['confirm','Do potwierdzenia']].map(([id,label])=>`<option value="${id}" ${id===filter?'selected':''}>${label}</option>`).join('')}</select><span class="filter-count">${rows.length} pozycji</span></div><section class="panel table-wrap"><table class="data-table"><thead><tr><th>Paczkomat / adres</th><th>Praca</th><th class="hide-mobile">Status</th><th></th></tr></thead><tbody>${rows.map(j=>{const l=state.locations[j.locationId],v=currentVisits().find(v=>v.jobs.some(a=>a.id===j.id));return `<tr class="${j.status==='done'?'row-done':''}"><td><strong>${j.status==='done'?`<span class="done-mark" title="Wykonano">${icon('checkCircle')}</span>`:''}${esc(locationTitle(l))}</strong><small>${esc(l.label)}</small></td><td>${esc(j.type)}${j.requiresConfirmation&&!j.confirmed?'<br><span class="tag amber">Potwierdź wyjazd</span>':''}${!validCoords(l)?'<br><span class="tag amber">Brak lokalizacji</span>':''}</td><td class="hide-mobile">${j.status==='done'?'<span class="tag green">Wykonano</span>':j.status==='doing'?'<span class="tag blue">W trakcie</span>':'Do wykonania'}</td><td><button class="small" data-act="visit" data-id="${esc(v.id)}">Otwórz</button></td></tr>`;}).join('')||'<tr><td colspan="4" style="padding:40px;text-align:center">Brak zleceń w tym widoku.</td></tr>'}</tbody></table></section>`;}
...
function drawMap(){
  ...
  for(const v of currentVisits()){
    ...
    const otherDay=inRoute?undefined:dayOfVisit.get(v.id),index=inRoute?route.findIndex(r=>r.id===v.id)+1:null;
    const dayClass=inRoute?`day${selectedDay}`:otherDay!==undefined?`day${otherDay}`:'candidate';
    const label=inRoute?(v.done?'✓':index):otherDay!==undefined?DAY_SHORT[otherDay]:'+';
...
document.addEventListener('click',async event=>{
  ...
  try{switch(action){
    ...
    case 'day':selectedDay=Number(button.dataset.day);addSectionExpanded=false;openStop=null;showDone=false;render();loadDayRoute();break;
