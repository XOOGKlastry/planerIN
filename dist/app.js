...
function nextStopCandidates(day){
  ...
  const route=dayRoute(day),placed=new Set(Object.values(currentPlan()?.routes||{}).flat());
...
// A yes/no dialog built on the same <dialog>: resolves once, however it gets dismissed (button or the header X).
function confirmModal(title,body,confirmLabel){
  return new Promise(resolve=>{
    let done=false;const finish=v=>{if(done)return;done=true;modal.removeEventListener('close',onClose);resolve(v);};
    const onClose=()=>finish(false);
    showModal(title,body,`<button data-act="close" id="confirm-no">Anuluj</button><button class="primary" id="confirm-yes">${esc(confirmLabel)}</button>`);
    modal.addEventListener('close',onClose);
    $('#confirm-no').onclick=()=>{finish(false);modal.close();};
    $('#confirm-yes').onclick=()=>{finish(true);modal.close();};
  });
}
function render(){
  ...
  root.innerHTML=`<div class="shell"><aside class="sidebar"><div class="brand"><span class="brand-mark">P</span><span>PaczkoPlan<small>Plan ekipy</small></span></div><nav class="side-nav">${navHtml(false)}</nav><div class="side-bottom"><button data-act="export">${icon('share')}Przekaż plan</button><button data-act="install">${icon('download')}Zainstaluj</button><div class="side-note">Zapis na tym urządzeniu.<br>Nawigacja w Google Maps.</div></div></aside><main class="main"><header class="topbar"><div class="header-title"><div class="eyebrow">PaczkoPlan / ekipa w terenie</div><h1>${titles[view]}</h1><p>${view==='week'?`${formatDate(state.settings.week)} – ${formatDate(addDays(state.settings.week,6))} ${state.settings.week.slice(0,4)}`:view==='jobs'?`${jobs.length} zleceń · ${vs.length} wizyt`:view==='import'?'Excel lub zapisany plan od kolegi':'Miejsce startu i Google Maps'}</p></div>${view==='week'?`<div class="top-actions"><button data-act="choose-excel">${icon('upload')}Importuj Excel</button><button class="primary" data-act="refresh-matrix" ${busy||!jobs.length?'disabled':''}>${icon('route')}${busy?'Liczę…':'Policz odległości'}</button></div>`:view==='jobs'?`<button class="primary" data-act="add-job">${icon('plus')}Dodaj zlecenie</button>`:''}</header>${!navigator.onLine?'<div class="offline-banner">Tryb offline. Zapisany plan i opisy są dostępne. Nowe odległości wymagają połączenia.</div>':''}${saveError?notice(esc(saveError),'<button class="small" data-act="export">Pobierz kopię</button>','error'):''}${waitingWorker?'<div class="update-bar">Dostępna nowa wersja.<button class="small" data-act="update">Zaktualizuj</button></div>':''}${view==='week'?renderWeek(vs,p,unready):view==='jobs'?renderJobs(jobs):view==='import'?renderImport():renderSettings()}<div class="statusbar" style="margin-top:20px">${icon('lock')}<span>${state.updatedAt?`Zapis ${new Date(state.updatedAt).toLocaleTimeString('pl-PL',{hour:'2-digit',minute:'2-digit'})} na tym urządzeniu`:'Plan jest przechowywany na urządzeniu'}</span></div></main><nav class="mobile-nav">${navHtml(true)}</nav></div>`;
...
function renderAddSection(day){
  ...
  return `<section class="panel" style="margin-top:22px"><div class="panel-title"><h2>${heading} <span class="muted">${pool.length}</span></h2><button class="small" data-act="auto-distribute">${icon('route')}Zaproponuj rozkład</button></div>${body}</section>`;
}
...
async function refreshMatrix(){
  ...
  if(apiKey){
    const n=located.length+1;
    if(n>25)return toast(`Tryb Google obsługuje do 25 lokalizacji naraz (masz ${n}). Wybierz jedną ekipę, mniejszy tydzień, albo usuń klucz w Ustawieniach, żeby wrócić do darmowego OSRM.`);
    const elements=googleMatrixElementCount(n);
    const proceed=await confirmModal('Policzyć przez Google?',`<p>To zapytanie sprawdzi <strong>${n}×${n} = ${elements} par lokalizacji</strong> przez Google Distance Matrix — płatne powyżej darmowego limitu Twojego konta Google Cloud.</p><p class="privacy-text">Ustaw budżet i alert w Google Cloud Console, żeby mieć pewność, że się nie rozpędzi.</p>`,'Policz przez Google');
    if(!proceed)return;
  }
  busy=true;render();showModal('Liczę odległości',`<div class="progress-box"><strong>Pobieram ${apiKey?'czasy z Google':'drogowe czasy dojazdu'}…</strong><p>Między bazą a wszystkimi punktami tygodnia.</p></div>`);
  try{
    const matrix=apiKey?await fetchMatrixGoogle(state.settings.base,located,apiKey):await fetchMatrix(state.settings.base,located);
    const old=currentPlan();
    state.plans[planKey()]={routes:old?.routes||{},geometries:old?.geometries||{},overnight:old?.overnight||{},matrix};
    busy=false;modal.close();await save();toast(`Odległości policzone (${matrix.provider}). Sprawdź podpowiedzi przy każdym dniu.`);
...
async function autoDistributeAction(){
  const {vs,cost}=baseCostContext();
  if(!cost)return toast('Najpierw policz odległości.');
  const placed=new Set(Object.values(currentPlan()?.routes||{}).flat());
  const pool=currentVisits().filter(v=>!placed.has(v.id)&&validCoords(v.location)&&!v.blocked);
  if(!pool.length)return toast('Wszystkie zlecenia mają już przypisany dzień.');
  const poolIndices=pool.map(v=>vs.findIndex(a=>a.id===v.id)+1).filter(i=>i>0);
  const anchors=Object.fromEntries(ALL_DAYS.map(d=>[d,dayAnchorIndex(d,vs)]));
  const p=currentPlan()||{routes:{},geometries:{},overnight:{}};
  // autoDistribute works on matrix indices (like the rest of core.js); translate ids to indices going in
  // and back to ids coming out, since a plan's routes are stored as visit ids everywhere else.
  const existingIndices=Object.fromEntries(ALL_DAYS.map(d=>[d,(p.routes?.[d]||[]).map(id=>vs.findIndex(v=>v.id===id)+1).filter(i=>i>0)]));
  const {routes,leftover}=autoDistribute(ALL_DAYS,anchors,existingIndices,poolIndices,cost);
  p.routes=Object.fromEntries(ALL_DAYS.map(d=>[d,routes[d].map(i=>vs[i-1]?.id).filter(Boolean)]));
...
async function setStatus(id,status){const v=currentVisits().find(v=>v.id===id);if(!v)return;if(v.blocked)return toast('Potwierdź możliwość wyjazdu w szczegółach zlecenia.');const at=new Date().toISOString();for(const job of v.jobs){job.status=status;if(status==='doing')job.startedAt??=at;if(status==='done'){job.startedAt??=at;job.finishedAt=at;}}await save();toast(status==='done'?'Wizyta zakończona.':'Rozpoczęto pracę.');}
...
async function saveSettings(form){const googleApiKey=text(new FormData(form).get('googleApiKey'));Object.assign(state.settings,{googleApiKey});await save();toast('Ustawienia zapisane.');}
...
async function exportPlan(){
  ...
  // A Google key is per-device billing, not plan data — never let it hop to a colleague's phone.
  copy.settings={...copy.settings,googleApiKey:''};
